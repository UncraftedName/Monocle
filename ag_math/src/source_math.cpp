...
bool Portal::ShouldTeleport(const VPlane& portal_plane, const Vector& ent_center, bool check_portal_hole)
{
    assert(!check_portal_hole);
    return Portal_EntBehindPlane(&portal_plane, &ent_center);
}

Vector Portal::TeleportNonPlayerEntity(const VMatrix& mat, const Vector& pt)
{
    Vector v;
...
void PortalPair::CalcPrimaryTeleportMatrix(const matrix3x4_t& localToWorld,
...
{
    if (p1_to_p2) {
        CalcPrimaryTeleportMatrix(p1_mat, p2_mat, out);
    } else {
        VMatrix p1_to_p2_mat;
        CalcPrimaryTeleportMatrix(p1_mat, p2_mat, p1_to_p2_mat);
